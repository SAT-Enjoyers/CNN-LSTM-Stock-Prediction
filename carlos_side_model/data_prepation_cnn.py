...
def get_all_time_series(df):
    ...
    for name in unique_names:
        ...
        adjusted_length = ts_length - ts_length % 64
        print(ts_length,adjusted_length)
...
def partitionTimeSeries(ts_unsplit, ts_split):
    ...
    partitions = np.split(ts_unsplit, ts_unsplit.shape[0] // 64)
...
# np.save('carlos_side_model/data_prep_out', all_time_series)

# Get the recurrence plots for all the time series
all_time_series = np.load("carlos_side_model/data_prep_out.npy")
