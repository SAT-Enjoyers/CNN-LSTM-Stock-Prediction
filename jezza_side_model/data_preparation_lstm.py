...
def get_all_time_series(df):
    ...
    return ([ts_open, ts_high, ts_low, ts_vol], ts_close)
...
X, y = get_all_time_series(df)

X = np.array(X)
y = np.array(y)

FILEPATH = 'jezza_side_model/'
np.save(FILEPATH + 'data_prep_out_X', X)
np.save(FILEPATH + 'data_prep_out_y', y)
